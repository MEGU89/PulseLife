...
import { CalendarDays, Clock3, Droplets, Hospital, MapPinned, Phone } from "lucide-react";
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="space-y-2">
          ...
          <h3 className="text-2xl font-black tracking-tight text-slate-950">
            {`${request.bloodType || "Blood"} blood request`}
          </h3>
          <p className="text-sm leading-7 text-slate-600">
            {request.recipientName
              ? `Created for ${request.recipientName}.`
              : "Created inside Pulse Bank for emergency support."}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <StatusBadge value={titleCase(request.urgency)} tone={getTone(request.urgency)} />
          <StatusBadge value={titleCase(request.status)} tone={getTone(request.status)} />
  ...
  return (
    <Panel className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h3 className="text-2xl font-black tracking-tight text-slate-950">
            {request?.hospital || schedule.donorName || "Donation schedule"}
          </h3>
          <p className="mt-2 text-sm leading-7 text-slate-600">
            {request?.bloodType ? `${request.bloodType} blood request` : "Scheduled Pulse Bank donation"}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
